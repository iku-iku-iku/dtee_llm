...
#define llm_inference __secure_llm_inference_impl
...
static int run(const char* prompt, int n_predict, char* out_res, int out_res_len)
{
    gpt_params params;
    int out_res_idx = 0;
    params.n_threads = 1;
    ...
    while (n_remain != 0) {
        ...
        for (auto id : embd) {
            ...
            eapp_print("[TOKEN GENERATED] %s", token_str.c_str());
            int token_str_idx = 0;
            while (out_res_idx < out_res_len && token_str_idx < token_str.size()) {
                out_res[out_res_idx++] = token_str[token_str_idx++];
            }
        }
        // fflush(stdout);
...
int llm_inference(char* in_prompt, int in_prompt_len, int n_predict, out_char out_res[OUT_SIZE])
{
    ...
    run(in_prompt, n_predict, out_res, OUT_SIZE);
    // eapp_print("RES: %s\n", out_res);

    // eapp_print("MAX CNT: %d\n", max_cnt);
    #ifdef USE_TLSF
    tlsf_destroy(g_tlsf_pool);
    #endif
    return 0;
}
